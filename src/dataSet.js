import { readFixedString, readFixedStringUTF8 } from './byteArrayParser.js';
...
export default class DataSet {
  ...
  string (tag, index) {
	...
	if (iso_IR_element) {
      var iso_IR_code = readFixedString(this.byteArray, iso_IR_element.dataOffset, iso_IR_element.length);
    } // end if iso_IR_element

    if (element && element.length > 0) {
      var fixedString = '';		
      if (iso_IR_code && iso_IR_code == 'ISO_IR 192') {
        fixedString = readFixedStringUTF8(this.byteArray, element.dataOffset, element.length);
      } else {
        fixedString = readFixedString(this.byteArray, element.dataOffset, element.length);		  
